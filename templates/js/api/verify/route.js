...
export async function POST(req) {
  try {
    ...
    const websiteUrl = new URL(req.url).origin;
    ...
    try {
      response = await axios.post(
        wsServerUrl,
        {
          droplertId: process.env.NEXT_PUBLIC_DROPLERT_ID,
          websiteUrl,
        },
        {
          headers: {
            "Content-Type": "application/json",
            apiKey: process.env.DROPLERT_KEY,
          },
        }
      );
    } catch (err) {
    ...
    if (response && response.status === 200) {
      ...
      console.error(
        "WebSocket server failed to subscribe the website:",
